---
title: "Módulo Redes Sociales: Sesión 7"
...
if(!require(sand)){
  install.packages("sand"); library(igraphdata)}
```
...
## Ejemplo: Interacciones sociales {.smaller}
...
```{r}
# algoritmos
kc_fast_greedy <- cluster_fast_greedy(g,)
kc_leading_eigen <- cluster_leading_eigen(g)
kc_walktrap <- cluster_walktrap(g)
kc_louvain <- cluster_louvain(g)
kc_label_prop <- cluster_label_prop(g)
kc_spinglass <- cluster_spinglass(g)
# kc_optimal <- cluster_optimal(g) el más pesado computacionalmente hablando
kc_infomap <- cluster_infomap(g)
```
...
l <- layout_with_kk(g)
par(mfrow = c(1,4))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_fast_greedy$membership, main = paste0("fast greedy: ", "Mod = ", round(modularity(kc_fast_greedy), 4)))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_leading_eigen$membership, main = paste0("leading eigen: ", "Mod = ", round(modularity(kc_leading_eigen), 4)))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_walktrap$membership, main = paste0("walktrap: ", "Mod = ", round(modularity(kc_walktrap), 4)))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_louvain$membership, main = paste0("louvain: ", "Mod = ", round(modularity(kc_louvain), 4)))
```
...
#| code-fold: true
par(mfrow = c(1,3))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_label_prop$membership, main = paste0("label prop: ", "Mod = ", round(modularity(kc_label_prop), 4)))
plot(g, vertex.label = NA, layout = l, vertex.color = kc_spinglass$membership, main = paste0("spinglass: ", "Mod = ", round(modularity(kc_spinglass), 4)))
#plot(g, vertex.label = NA, layout = l, vertex.color = kc_optimal$membership, main = paste0("optimal: ", "Mod = ", round(modularity(kc_optimal), 4)))
...
grupos <- as.numeric(as.factor(vertex_attr(graph = g, name = "Grupo")))
table(grupos)
...
```{r}
# comparación
compare(comm1 = grupos, comm2 = kc_fast_greedy$membership, method = "rand")
compare(comm1 = grupos, comm2 = kc_leading_eigen$membership, method = "rand")
```
...
```{r}
# tablas cruzadas
table(grupos, kc_fast_greedy$membership)
table(grupos, kc_leading_eigen$membership)
```

##  Ejemplo: Simulación {.smaller}
...
## Ejemplo: Simulación (cont.) {.smaller}
